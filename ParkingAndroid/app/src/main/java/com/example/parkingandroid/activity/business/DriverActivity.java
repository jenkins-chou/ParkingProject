...
public class DriverActivity extends BaseEmptyActivity implements View.OnClickListener{
    ...
    public void initView() {
        ...
        award_time = findViewById(R.id.award_time);
        validity = findViewById(R.id.validity);
        awrad_unit = findViewById(R.id.awrad_unit);
...
//                .recordVideoSecond()//视频秒数录制 默认60s int
    ...
    void submitData(){
        ...
        params.put("award_time",award_time.getText().toString() + "");
        params.put("validity",validity.getText().toString()+ "");
        params.put("awrad_unit",awrad_unit.getText().toString()+ "");
        params.put("number",number.getText().toString()+ "");
        if (!TextUtils.isEmpty(imageUrl)){
            params.put("photo",imageUrl+ "");
        }
        if (driverModel != null){
            params.put("id",driverModel.getId()+ "");
        }
        driverPresenter.setDriver(params);
    }

    void initPresenter(){
        driverPresenter = new DriverPresenter(context);
        driverPresenter.setOnCallBack(new DriverPresenter.OnCallBack() {
            @Override
            public void getDriver(boolean isSuccess, Object object) {
                if (isSuccess && object != null){
                    ResultModel resultModel = (ResultModel)object;
                    if (resultModel != null && TextUtils.equals(resultModel.getCode(),Const.KEY_RES_CODE_200)){
                        List<DriverModel> driverModels = resultModel.getData();
                        if (driverModels != null && driverModels.size()>=1){
                            driverModel = driverModels.get(0);
                            showDriverInfo(driverModel);
                        }
                    }
                }
            }
            ...
            @Override
            public void setDriver(boolean isSuccess, Object object) {
                if (isSuccess && object != null) {
                    ResultModel resultModel = (ResultModel) object;
                    if (resultModel != null && TextUtils.equals(resultModel.getCode(), Const.KEY_RES_CODE_200)) {
                        Toast.makeText(context, "更新成功", Toast.LENGTH_SHORT).show();
                        driverPresenter.getDriver(RS.getBaseParams(DriverActivity.this));
                    }
                }
            }
        });
        driverPresenter.getDriver(RS.getBaseParams(this));
    }

    void showDriverInfo(DriverModel driverModel){
        if (driverModel != null){
            award_time.setText(driverModel.getAward_time());
            validity.setText(driverModel.getValidity());
            awrad_unit.setText(driverModel.getAwrad_unit());
            number.setText(driverModel.getNumber());
            RequestOptions requestOptions = new RequestOptions();
            requestOptions.error(R.mipmap.icon_card);
            requestOptions.placeholder(R.mipmap.icon_card);
            Glide.with(context).load(BaseAPI.base_url+driverModel.getPhoto()).apply(requestOptions).into(photo);
        }
    }
